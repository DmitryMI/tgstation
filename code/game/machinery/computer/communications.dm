...
/obj/machinery/computer/communications/expedition/ui_data(mob/user)
	var/list/data = ..()
	if (!data["authenticated"] || data["page"] != STATE_MAIN)
		return data
...
/obj/machinery/computer/communications/proc/receives_message(datum/comm_message/message)
...
/datum/comm_message/New(new_title,new_content,new_possible_answers, new_message_source = COMM_MESSAGE_SOURCE_MISC)
	...
	message_source = new_message_source
